...
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'webkb', 'Dataset string.')  # 'webkb', 'citeseer', 'pubmed'
...
# Define placeholders
placeholders = {
    'features': tf2.sparse_placeholder(tf.float32, shape=tf.constant(features[2], dtype=tf.int64)),
    'labels': tf2.placeholder(tf.float32, shape=(None, y_train.shape[1])),
    'labels_mask': tf2.placeholder(tf.int32),
    'dropout': tf2.placeholder_with_default(0., shape=()),
    'num_features_nonzero': tf2.placeholder(tf.int32)  # helper variable for sparse dropout
}
